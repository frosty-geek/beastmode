...
export async function copyPlugin({ homeDir, packageDir, version }) {
  ...
  // Link plugin content into marketplace dir so "source": "./plugin" resolves.
  // Windows: use 'junction' (no admin rights required for directories).
  // Unix: use default symlink.
  const linkTarget = join(marketplaceDir, 'plugin');
  const linkType = process.platform === 'win32' ? 'junction' : undefined;
  await symlink(cacheDir, linkTarget, linkType);
