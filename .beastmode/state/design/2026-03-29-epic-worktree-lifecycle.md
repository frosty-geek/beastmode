...
- **Release boundary**: Skills can do any git operations inside the worktree (commit, tag, etc.) — they just don't know it's a worktree. CLI owns worktree container operations: squash-merge to main, archive tag, worktree removal. The "TRANSITION BOUNDARY" block in the release skill is deleted — the skill never leaves the worktree.
...
- **Skill cleanup**: Full sweep of ~16 skill files. Reword "worktree directory name" to "feature slug". Remove `@worktree-manager.md` imports. Remove release checkpoint steps that cross the worktree boundary (squash-merge, transition boundary). Update references to Justfile.
