...
## Decision
Two-tier system. HARD-GATE (XML tags) for unconditional constraints. Configurable gates use `## N. [GATE|namespace.gate-id]` with `[GATE-OPTION|human]` and `[GATE-OPTION|auto]` subsections. Config resolved from `.beastmode/config.yaml` at runtime. @imports between knowledge hierarchy levels (L0/L1/L2/L3) prohibited; cross-skill @imports from `skills/_shared/` are allowed. Transition gates use standardized output: human mode = `Next:` + inline-code command; auto/low-context = `Start a new session and run:` + inline-code command. All transition output ends with STOP — no additional prose or alternatives.
