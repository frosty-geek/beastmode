...
## Decision
Extend `config.yaml` with `transitions:` block (backlog-to-design through release-to-done, each set to human or auto) and `github:` section with `enabled` key (boolean, default false) and `project-name` key. When `github.enabled` is false, all GitHub checkpoint steps are silently skipped. Setup subcommand sets `enabled` to true. Transitions block replaces implicit gate-only configuration for phase advancement.
