...
- `store.json` is the structured entity store at `.beastmode/state/store.json` (gitignored, coexists with manifests during PRD-1; absorbs manifests in PRD-2) — provides hash-based IDs, cross-epic dependency modeling, and queryable CRUD via `beastmode store` CLI namespace. Store owns slug derivation: `InMemoryTaskStore.deriveEpicSlug(name, id)` is the single source of truth for the `{slugify(name)}-{hex}` format; `updateEpic()` auto-derives slug when name changes; `EpicPatch` excludes `slug` to prevent external mutation. `addPlaceholderEpic()` generates hacker-jargon placeholders for the design phase entry point
