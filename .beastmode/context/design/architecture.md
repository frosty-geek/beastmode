...
- Manifest JSON is the operational authority for feature lifecycle with top-level `phase` field as the single phase source of truth; GitHub is a synced mirror updated at checkpoint boundaries when enabled — repo files remain the content store
