...
## Decision
Each dispatched SDK session gets a ring buffer of ~100 recent log entries. Buffers collect continuously via the EventEmitter, even when the user is viewing a different session. Navigating to any session shows its buffer contents immediately.
