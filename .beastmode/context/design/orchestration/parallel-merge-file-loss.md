...
## Decision
After all parallel feature impl branches are merged, run a file presence verification pass against the expected output tree (from the Write Tasks file structure tables) before checkpoint. If files are missing, restore from the originating impl branch before it is deleted.
