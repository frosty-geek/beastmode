...
- Three data domains: Artifacts (committed skill outputs in `artifacts/`), State (gitignored pipeline state in `state/`), Context (published knowledge). The JSON file store (`store/json-file-store.ts`) is the sole operational authority for feature lifecycle; GitHub is a one-way synced mirror updated by the CLI after every phase dispatch when enabled. Store owns slug derivation: `InMemoryTaskStore.deriveEpicSlug(name, id)` is the single source of truth for the `{slugify(name)}-{hex}` format; `updateEpic()` auto-derives slug when name changes; `EpicPatch` excludes `slug` to prevent external mutation. `addPlaceholderEpic()` generates hacker-jargon placeholders for design phase entry. Slug format: `slugify(name)-{4-hex-chars}` for epics (`slugify(epicName)--slugify(featureName)-{ordinal}` for features, embedding the parent epic name with `--` separator). Design-phase initial slug uses placeholder name; renamed to human-readable form at reconciliation when design skill provides `epic-name`. Old-format slugs remain valid
...
3. ALWAYS rely on wave file isolation for parallel implementation -- agents commit directly to the shared feature branch, no merge step needed
...
## CLI Architecture
TypeScript CLI (`beastmode`) built with Bun and Claude Agent SDK that provides manual phase execution (`beastmode <phase> <slug>`) with phase detection matrix (regression, same-phase rerun, forward, forward-jump blocking), fullscreen TUI dashboard (`beastmode dashboard`) with embedded WatchLoop for autonomous pipeline orchestration, standalone context tree compaction (`beastmode compact`), and hook dispatch (`beastmode hooks <name> [phase]`) routing to hook handler functions as pure library imports. CLI-managed git tags (`beastmode/<slug>/<phase>`) provide deterministic reset targets for phase regression — tags are created at phase checkpoint, deleted on regression, and renamed during slug rename. Lives in `cli/` with its own `package.json`, separate from the plugin's markdown skills. Owns worktree lifecycle, store lifecycle, and GitHub sync. Store is a JSON file store (`store/json-file-store.ts`) — the sole persistence layer for epic/feature state. Store entities include Epic (with slug, name, phase, features, sync refs) and Feature (with name, slug, status, reDispatchCount). `store.find()` resolves by exact ID, exact slug, or prefix slug (opt-in via `resolveIdentifier()` with `allowPrefix: true`). Resolution chain: exact ID -> exact slug -> prefix slug (opt-in). Typed accessors `getEpic()` and `getFeature()` replace the removed `store.find()` for direct lookups. Single `store.save()` per dispatch — machine persist action accumulates state in memory only, `store.save()` is a pure write. After every phase dispatch, a Stop hook auto-generates output.json from artifact frontmatter (located by hex slug match), then the CLI enriches the store and runs GitHub sync. Non-design phases fail fast if slug not found. Store uses flat-file convention: `.beastmode/state/YYYY-MM-DD-<slug>.store.json`, local-only and gitignored. Skills write artifacts to `artifacts/<phase>/` only with standardized frontmatter (`phase`, `epic-id` entity identifier, `epic-slug` name always present; phase-specific additions per phase). Status data logic (sorting, filtering, snapshot building, change detection) extracted into shared `status-data.ts` module consumed by the dashboard. Dashboard command provides fullscreen Ink/React TUI with embedded watch loop. Log panel uses shared TreeView component for hierarchical tree rendering. Cost tracking removed from scanner and status. Runtime dependencies: `ink` (v6.8.0), `react`, and `chalk` added to cli/package.json for dashboard rendering and structured log formatting.
...
5. ALWAYS run unified pipeline via `pipeline/runner.ts` — both manual CLI and dashboard call the same runner: Step 0 entity creation via `store.addPlaceholderEpic()` (design with no slug) or lookup via `store.listEpics().find()` (all others), worktree prepare, rebase (skip for design), settings create, early issue creation (pre-dispatch), dispatch, artifact collect, store reconcile (pure store mutation), post-reconcile side effects for design (tag rename, artifact rename, worktree rename), store advance, GitHub mirror, git push, branch linking, worktree cleanup (release only)
...
9. ALWAYS use standardized frontmatter across all phase artifacts — `phase`, `epic-id` (entity identifier), `epic-slug` (identity echo) always present; design adds `epic-name` (DECISION — human-readable title for post-session rename); plan adds `feature-slug`, `wave`; implement adds `feature-id`, `feature-slug`, `status`; validate adds `status`, `failed-features`; release adds `bump`
...
Explicit contract governing YAML frontmatter in skill artifacts, session-stop pass-through, and reconcile content extraction. Unified field naming across env vars, metadata-in, frontmatter, and output.json. Content fields (problem, solution, description) live in artifact markdown body sections, not frontmatter. Session-stop is a dumb translator; reconcile is the brain. Design phase uses `epic-name` as the DECISION field (human-readable title); `epic-slug` is an identity echo in all phases. Session-stop passes `epic-name` through to `output.json` where reconcile reads it to update the store name (which auto-derives the new slug).

1. ALWAYS use unified field names across all layers — `epic-id`, `epic-slug`, `epic-name`, `feature-id`, `feature-slug`, `failed-features` are the canonical names in frontmatter and output.json
2. NEVER put content fields (problem, solution, description) in frontmatter — content lives in artifact markdown body sections and is extracted by reconcile
3. ALWAYS distinguish identity echoes from skill decisions in frontmatter — echoes repeat metadata-in values; design DECISION is `epic-name` (human-readable title), not `epic-slug`
4. ALWAYS treat session-stop as a dumb pass-through — no env var comparison, no identity/decision distinction, no rename logic
5. ALWAYS extract content from artifact body sections in reconcile — `## Problem Statement` + `## Solution` for epic.summary, `## What to Build` for feature.description

context/design/frontmatter-contract.md
...
## State Scanner
Scanning is composed from the store's `listEpics()` plus XState machine snapshots producing `EnrichedEpic[]`. No standalone scanner module. Store entities (Epic, Feature) are the sole state types. Store path: state/YYYY-MM-DD-<slug>.store.json (gitignored). Store owns slug derivation: `deriveEpicSlug(name, id)` produces `{slugify(name)}-{hex}` format; `updateEpic()` auto-derives slug when name changes; `EpicPatch` excludes `slug`. `addPlaceholderEpic()` generates hacker-jargon names for design phase. Design-phase initial slug uses placeholder name; renamed at reconciliation when `epic-name` is provided. Old-format slugs remain valid. Slugs embed unique suffixes, making deduplication unnecessary. Slug format validated via `isValidSlug()`.
...
6. ALWAYS validate slug format against `[a-z0-9](?:[a-z0-9.-]*[a-z0-9])?` via `isValidSlug()` — centralized in store module; accepts dots and the `--` epic-feature separator
