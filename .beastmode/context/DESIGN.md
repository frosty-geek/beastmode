...
- Three data domains: Artifacts (committed skill outputs in `artifacts/`), State (gitignored pipeline manifests and store in `state/`), Context (published knowledge). Manifest JSON is the operational authority for feature lifecycle via manifest-store.ts (filesystem boundary) and manifest.ts (pure state machine); `store.json` is the structured entity store providing hash-based IDs (`bm-xxxx`), cross-epic `depends_on` dependency modeling, and queryable CLI namespace (`beastmode store`), coexisting with manifests during PRD-1; GitHub is a one-way synced mirror updated by the CLI after every phase dispatch when enabled. Manifest terminology: `slug` is an immutable 6-character hex assigned at worktree creation, `epic` is the human-readable name derived after design phase rename, `feature` is a sub-unit within an epic
...
## Dashboard
Fullscreen terminal UI (`beastmode dashboard`) built with Ink v6.8.0 + React with three-panel k9s-style layout (ThreePanelLayout with EpicsPanel, OverviewPanel, LogPanel as slot children): epics list (top-left ~30%), overview (top-right ~70%), log (bottom ~65% full-width). No outer chrome border — panels have individual cyan single-line borders with inset titles and dark charcoal interior backgrounds. Header is a NyanBanner component — 2-line ASCII block art with continuously cycling nyan cat rainbow colors (80ms tick, 6-stripe palette). Epic list rows use status-aware icons (selected arrow, running spinner, phase-colored idle dot, dimmed done dot) with compact slug + phase badge format. Epics panel is the sole interactive panel — overview and log are passive displays reacting to epic selection. Flat navigation model with "(all)" aggregate entry at top of epic list. Keyboard navigation via use-dashboard-keyboard (up/down in epics list, filter mode, cancel confirmation, toggle done/cancelled). Dashboard honors the configured dispatch strategy (sdk/cmux/iterm2/auto) via `selectStrategy()` — same function used by the watch command. When SDK strategy is active, live message streaming is available; when non-SDK strategy is active, the log panel falls back to WatchLoop lifecycle event entries. Log verbosity is runtime-adjustable via `v` keypress (info → detail → debug → trace → info), with the current level shown in the key hints bar and filtering applied at render time. A message mapper converts SDKMessage types to terminal-friendly log entries (text deltas inline, tool calls as one-liners). Ring buffers per session (~100 entries) collect continuously so history is available immediately on navigation. Context-sensitive key hints in bottom bar. Embedded watch loop via EventEmitter typed events. Signal handling externalized from WatchLoop.
...
9. ALWAYS use ThreePanelLayout with EpicsPanel, OverviewPanel, LogPanel as slot children — flat navigation, no drill-down or view stack
10. ALWAYS use `selectStrategy(config.cli["dispatch-strategy"])` in both dashboard and watch commands — shared strategy selection ensures identical dispatch behavior across both UIs
...
12. ALWAYS use use-dashboard-keyboard for all keyboard input — single hook handles flat navigation, filter mode, cancel confirmation, toggle, and verbosity cycling
