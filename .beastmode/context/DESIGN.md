...
TypeScript CLI watch mode (`beastmode watch`) scans local state files and dispatches agent sessions in CLI-owned worktrees to drive epics through plan -> release in parallel. Dispatch uses a strategy pattern: `DispatchedSession` interface with `SdkSession` (SDK `query()`) and `CmuxSession` (cmux terminal surface) implementations. A `SessionFactory` selects the strategy based on cmux availability and config. No concurrency cap — API rate limits are the natural governor. Per-epic scoped logger instances via `createLogger(verbosity, epicSlug)`; system-level messages use `beastmode` as slug prefix. Fan-out per feature at implement with wave-gated dispatch — `dispatchFanOut()` dispatches only features from the lowest incomplete wave, wave N+1 starts only when all wave N features complete. Design phase is excluded (interactive). Respects config.yaml gates, pauses epic and logs to stdout on human gates. Event-driven re-scan on session completion with 60-second poll as safety net.
...
3. ALWAYS dispatch features wave-by-wave in dispatchFanOut() — find the lowest wave with pending/in-progress features, dispatch only from that wave, block subsequent waves until current wave completes
4. ALWAYS merge implement worktrees sequentially with pre-merge conflict simulation via `git merge-tree` — optimized merge order
5. ALWAYS respect config.yaml gate settings — human gates pause the epic and log to stdout, user runs `beastmode <phase> <slug>` manually to proceed
6. ALWAYS use CLI-owned worktrees — CLI creates before, merges after, removes when done
7. ALWAYS use `DispatchedSession` interface for dispatch — `SessionFactory` returns `SdkSession` or `CmuxSession` based on runtime state and config
8. ALWAYS reconcile cmux state on startup — adopt live surfaces, close dead ones, remove empty workspaces

context/design/orchestration.md

## CLI Architecture
TypeScript CLI (`beastmode`) built with Bun and Claude Agent SDK that provides manual phase execution (`beastmode <phase> <slug>`) and autonomous pipeline orchestration (`beastmode watch`). Lives in `cli/` with its own `package.json`, separate from the plugin's markdown skills. Owns worktree lifecycle, manifest lifecycle, and GitHub sync. Manifest logic split into two modules: manifest-store.ts (filesystem boundary — get, list, save, create, validate) and manifest.ts (pure state machine — enrich, advancePhase, regressPhase, markFeature, cancel, deriveNextAction, checkBlocked, shouldAdvance). After every phase dispatch, a Stop hook auto-generates output.json from artifact frontmatter, then the CLI enriches the manifest and runs syncGitHub(manifest, config). Manifest uses flat-file convention: `.beastmode/state/YYYY-MM-DD-<slug>.manifest.json`, local-only and gitignored. `ManifestFeature` includes `wave: number` (default 1 for backwards compatibility) stamped by plan validate from feature frontmatter. Skills write artifacts to `artifacts/<phase>/` only. Status command provides compact table (Epic | Phase | Features | Status) with wave progress indicator (e.g., `W1/3`) in default mode, --verbose flag expanding to per-wave rows with feature counts and statuses, and a `--watch`/`-w` live dashboard mode that polls every 2 seconds, redraws via ANSI escape sequences, highlights changed rows for one cycle, and shows watch loop running state via lockfile detection. Cost tracking removed from scanner and status. GatesConfig extended with validate phase. Centralized logger module (`logger.ts`) with `createLogger(verbosity, slug)` factory, four verbosity tiers (0-3) controlled by `-v` flag stacking, stderr/stdout stream split, consistent `HH:MM:SS slug: message` format replacing all ad-hoc console output.
...
11. ALWAYS use createLogger(verbosity, slug) for all CLI output — no direct console.log/error, centralized logger is the sole output path
12. ALWAYS route errors and warnings to stderr, info and debug to stdout — stream separation enables clean piping
13. ALWAYS default to verbosity 0 (quiet) across all commands — state transitions and errors only, no special casing between manual and watch mode
