...
const TEST_PROJECT = "/tmp/test-push-integration";
...
const mockCreate = vi.hoisted(() => vi.fn(async (slug: string) => ({
  slug,
  path: `${TEST_PROJECT}/.claude/worktrees/${slug}`,
  branch: `feature/${slug}`,
})));
...
describe("@github-sync-polish: git-push integration", () => {
  beforeEach(() => {
    mkdirSync(`${TEST_PROJECT}/.claude/worktrees/test-epic/.claude`, { recursive: true });
